...
class XMLElement(object):
            ...
            for key in self.__dict__['order']:
                ...
                if value is not None:
                    if(type(value) is str or type(value) is str):
                        ...
                        raise TypeError("Generate method in class " + self.__class__.__name__ +
                                        " can not generate supplied type")
        else:
        ...
        if(name == "text"):
            if(type(value) == str or type(value) == str):
                if(self._validateValue(name, value)):
                    ...
                    ValueError("Value " + value + " (" + type(value).__name__ + ") is not valid as text of " +
            ...
            if(self._validateValue(name, value)):
                ...
                raise ValueError("Value " + str(value) + " (" + type(value).__name__ + ") is not valid for " +
            ...
            if(isinstance(validator, XMLValidator)):
                if(isinstance(validator, XMLValidatorRequired)):
                    ...
                    if(not self._validateValue(name, self.__dict__["text"])):
                        raise ValueError("Value " + self.__dict__["text"] + " (" +
            ...
            if(isinstance(validator, XMLValidator)):
                ...
                if(isinstance(validator, XMLValidatorRequired)):
                    ...
                    if(not self._validateValue(name, self.__dict__["items"][name])):
                        raise ValueError("Value " + self.__dict__["items"][name] +
    ...
    def _validateValue(self, name, value):
        ...
        if(name == "text"):
            ...
            if(name not in self.__dict__["order"]):
                raise NameError("This object (of class " +
                                self.__class__.__name__ + ") does not have attribute with given value")
...
class FiskSOAPClient(object):
    ...
    def send(self, message, raw=False):
        ...
        responseXML = et.fromstring(response.replace('''<?xml version="1.0" encoding="UTF-8"?>''', ""))
...
class Signer(object):
    ...
    def __init__(self, key, password):
        ...
        self.init_error = []
        self.key = crypto.dump_privatekey(crypto.FILETYPE_PEM, p12.get_privatekey())
...
class FiskInit():
    ...
    @staticmethod
    def init(key_file, password, production=False):
        """
        ...
        FiskInit.signer = Signer(key_file, password)
        FiskInit.isset = True
...
class EchoRequest(FiskXMLRequest):
    ...
    def __init__(self, text=None):
        ...
        """
        FiskXMLRequest.__init__(self, text=text,
                                childrenNames=(("text", [XMLValidatorLen(1, 1000), XMLValidatorRequired()]), ))
...
def zastitni_kod(oib, datumVrijeme, brRacuna, ozPoslovnogP, ozUredaja, ukupnoIznos, key_filename, key_password):
    ...
    p12 = crypto.load_pkcs12(open(key_filename, 'rb').read(), key_password)

    key = RSA.importKey(crypto.dump_privatekey(crypto.FILETYPE_PEM, p12.get_privatekey()), key_password)
